...
-- ; from = λ{ x → case-⊎ (λ x₁ → inj₂ x₁) (λ x₁ → inj₁ x₁) x }
...
-- ; case-⊎ (case-⊎ inj₁ (inj₂ ∘ inj₁)) (inj₂ ∘ inj₂)
    ; from = λ{
         (inj₁ x)        → inj₁ (inj₁ x)
       ; (inj₂ (inj₁ x)) → inj₁ (inj₂ x)
       ; (inj₂ (inj₂ x)) → inj₂ x
       }
-- ; case-⊎ (inj₁ ∘ inj₂) (case-⊎ (inj₁ ∘ inj₂) inj₂) x
    ; from∘to = λ{
         (inj₁ (inj₁ x)) → refl
       ; (inj₁ (inj₂ x)) → refl
       ; (inj₂ x)        → refl
...
\begin{code}
⊎-weak-x : ∀ {A B C : Set} → (A ⊎ B) × C → A ⊎ (B × C)
⊎-weak-x ⟨ inj₁ x , x₁ ⟩ = inj₁ x
⊎-weak-x ⟨ inj₂ x , x₁ ⟩ = inj₂ ⟨ x , x₁ ⟩
\end{code}
