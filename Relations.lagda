...
*-mono′-≤ : ∀ (m n p q : ℕ)
  ...
  → p ≤ q
    -------------
  → m * p ≤ n * q
*-mono′-≤ zero    n       p q z≤n       _   = z≤n
*-mono′-≤ (suc m) (suc n) p q (s≤s m≤n) p≤q = +-mono-≤ p q (m * p) (n * q) p≤q (*-mono′-≤ m n p q m≤n p≤q)
\end{code}
...
<-trans (s<s x) (s<s n<p) = s<s (<-trans x n<p)

<-trans′ : ∀ (m n p : ℕ) → m < n → n < p → m < p
<-trans′ zero    (suc _)  (suc _)  z<s       (s<s n<p) = z<s
<-trans′ (suc n) (suc n₁) (suc n₂) (s<s m<n) (s<s n<p) = s<s (<-trans′ n n₁ n₂ m<n n<p)
\end{code}
...
--     -----
--   → m < p
-- <-trans-revisited zero    _       _       z<s       (s≤s n≤p) = z<s
-- <-trans-revisited (suc m) (suc n) (suc p) (s<s m<n) (s≤s n≤p) = s<s (<-trans-revisited m n p m<n n≤p)
