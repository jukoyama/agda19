...
map-compose′ f g (x ∷ xs) =
  begin
    g (f x) ∷ map (g ∘ f) xs
  ≡⟨ cong (_∷_ (g (f x))) (map-compose′ f g xs) ⟩
...
extensionality : ∀ {A B : Set} {f g : A → B} → (∀ (x : A) → f x ≡ g x)
...
map-is-foldr′ {A} {B} {f} (x ∷ ys) =
  begin
    f x ∷ map f ys
  ≡⟨ cong (_∷_ (f x)) (map-is-foldr′ ys) ⟩
    f x ∷ (foldr (λ x xs → f x ∷ xs) []) ys
  ∎
...
--   map-Tree f g t ≡ fold-Tree (leaf ∘ f) (λ → )
\end{code}
