...
<-trichotomy-n : ∀ (m n : ℕ) → Tri (m < n) (m ≡ n) (n < m)
<-trichotomy-n zero    zero    = tri≡ (λ ())  refl    (λ ())
<-trichotomy-n zero    (suc n) = tri< z<s     (λ ()) (λ ())
<-trichotomy-n (suc m) zero    = tri> (λ ())  (λ ())  z<s
<-trichotomy-n (suc m) (suc n) with <-trichotomy-n m n
... | tri< m<n m≢n n≮m = tri< (s<s m<n)               (λ x → m≢n (cong pred x)) λ x → n≮m (<-pred x)
... | tri≡ m≮n m≡n n≮m = tri≡ (λ x → m≮n (<-pred x)) (cong suc m≡n)             λ x → n≮m (<-pred x)
... | tri> m≮n m≢n n<m = tri> (λ x → m≮n (<-pred x)) (λ x → m≢n (cong pred x)) (s<s n<m)
...
⊎-dual-x =
  record
    { to   = λ{ ¬AB         → ⟨ (λ a → ¬AB (inj₁ a)) , (λ b → ¬AB (inj₂ b)) ⟩ }
    ; from = λ{ ⟨ ¬A , ¬B ⟩ → λ{ (inj₁ A) → ¬A A
                                 ; (inj₂ B) → ¬B B
                                 }
               }
    ; from∘to = λ{ ¬AB → extensionality λ{ (inj₁ x) → refl
                                           ; (inj₂ y) → refl
                                           }
                  }
    ; to∘from = λ{ ⟨ ¬A , ¬B ⟩ → refl }
